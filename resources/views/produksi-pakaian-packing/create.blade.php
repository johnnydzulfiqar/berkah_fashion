...
@section('title1')
    Form Input Produksi Harian Bagian Packing
@endsection
...
@section('main')
    ...
    @endif
    <form action="{{ route('produksi-pakaian-packing.store') }}" method="post">
        @csrf
        <div id="detail-form-container">
            <div class="detail-form">
                ...
                <select name="kode_rencanaproduksi" required style="background-color: rgb(245, 241, 241)">
                ...
                <label for="total_berhasil_darijahit">Patokan Jumlah Pakaian Yang Telah Selesai Dari Bag.Jahit</label>
                <input type="text" name="total_berhasil_darijahit" id="total_berhasil_darijahit" required readonly >

                <label for="total_produksi_pakaian">Jumlah Pakaian Yang Telah Selesai Di Packing</label>
                ...
                <label for="sisaBelumDiproduksi">Jumlah Pakaian Yang Belum Di Packing</label>
                <input type="number" name="sisaBelumDiproduksi" id="sisaBelumDiproduksi" value=0 readonly
                    style="background-color: rgb(245, 241, 241)" required>

                <label for="jumlah_berhasil" >Inputkan Jumlah Pakaian Berhasil Di Packing:</label>
                <input type="text" name="jumlah_berhasil" id="jumlah_berhasil" value="{{ old('jumlah_berhasil') }}" required style="background-color: rgb(104, 238, 86)">
                @error('jumlah_berhasil')
                    <div class="alert alert-danger">{{ $message }}</div>
                @enderror

                <label for="jumlah_gagal">Inputkan Jumlah Pakaian Gagal Di Packing:</label>
                <input type="text" name="jumlah_gagal" id="jumlah_gagal" value="{{ old('jumlah_gagal') }}" required style="background-color:rgb(104, 238, 86)">
                @error('jumlah_gagal')
                    <div class="alert alert-danger">{{ $message }}</div>
                @enderror

                <label for="total_berhasildangagal">Total Berhasil dan Gagal Packing:</label>
                ...
                @elseif (Auth::user()->role == 'jahit')
                    <label for="biaya_jahit">Upah Kerja Jahit:</label>
                    <input type="number" name="biaya_jahit" id="biaya_jahit" required readonly>
                @elseif (Auth::user()->role == 'packing')
                    <label for="biaya_packing">Upah Kerja Packing (per pakaian) Rp. :</label>
    ...
    <script>
        $('select[name="kode_rencanaproduksi"]').on('change', function() {
            ...
            $.ajax({
                url: '/getRencanaProduksiDataPacking/' + kodeRencanaProduksi,
                type: 'GET',
                dataType: 'json',
        ...
        function updateFormWithRencanaProduksiData(data) {
            ...
            $('input[name="total_berhasil_darijahit"]').val(data.data.total_berhasil_darijahit);
        ...
        function updateTotalBerhasilDangagal() {
            ...
            var jumlahPatokanProduksi = parseFloat(document.getElementById('total_berhasil_darijahit').value) || 0;
            ...
            if (totalBerhasilDangagal > nilaiSementara) {
                //Total Berhasil + Gagal tidak boleh melebihi Jumlah Berhasil Dari bagian Jahit - Total Produksi Pakaian.
                alert('Tidak Boleh Melebihi Jumlah Pakaian Yang Belum Di Packing');
            ...
            //Update selisih
            document.getElementById('nilai_sementara').value = nilaiSementara;
        }
    </script>
@endsection
