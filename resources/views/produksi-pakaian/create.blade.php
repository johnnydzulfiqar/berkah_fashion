...
@section('title1')
    Form Input Harian Produksi Bagian Cutting
@endsection
...
@section('main')
    ...
    @endif
    <form action="{{ route('produksi-pakaian.store') }}" method="post">
        @csrf
        <div id="detail-form-container">
            <div class="detail-form">
                ...
                <label for="kode_rencanaproduksi">Rencana Produksi:</label>
                ...
                <input type="text" name="tanggal_awal" id="tanggal_awal" readonly required>
                {{-- Formulir milik tabel  produksi pakaian --}}
                ...
                <input type="text" name="nama_produk" id="nama_produk" readonly required>

                <label for="warna_produk">Warna:</label>
                <input type="text" name="warna_produk" id="warna_produk"  readonly required>

                <label for="ukuran_produk">Ukuran:</label>
                <input type="text" name="ukuran_produk" id="ukuran_produk"  readonly required>

                <label for="jumlah_rencanaproduksi">Patokan Jumlah Pakaian Dari Rencana Produksi Pemilik :</label>
                <input type="text" name="jumlah_rencanaproduksi" id="jumlah_rencanaproduksi"  readonly required>

                <label for="total_produksi_pakaian">Jumlah Pakaian Yang Telah Selesai Di Cutting</label>
                <input type="number" name="total_produksi_pakaian" id="total_produksi_pakaian" value=0  readonly required>

                <label for="sisaBelumDiproduksi">Jumlah Pakaian Yang Belum Di Cutting</label>
                ...
                <label for="jumlah_berhasil" >Inputkan Jumlah Pakaian Berhasil Di Cutting:</label>
                <input type="text" name="jumlah_berhasil" id="jumlah_berhasil" value="{{ old('jumlah_berhasil') }}"  style="background-color:  rgb(104, 238, 86)" required>
                @error('jumlah_berhasil')
                    <div class="alert alert-danger">{{ $message }}</div>
                @enderror

                <label for="jumlah_gagal">Inputkan Jumlah Pakaian Gagal Di Cutting:</label>
                <input type="text" name="jumlah_gagal" id="jumlah_gagal" value="{{ old('jumlah_gagal') }}"  style="background-color:  rgb(104, 238, 86)" required>
                @error('jumlah_gagal')
                    <div class="alert alert-danger">{{ $message }}</div>
                @enderror

                <label for="total_berhasildangagal">Total Berhasil dan Gagal:</label>
                <input type="number" name="total_berhasildangagal" id="total_berhasildangagal" readonly required>

                @if (Auth::user()->role == 'cutting')
                    <label for="biaya_cutting">Upah Kerja Cutting (per potong) (Rp.):</label>
                    <input type="number" name="biaya_cutting" id="biaya_cutting" required readonly required>
                @elseif (Auth::user()->role == 'jahit')
                    <label for="biaya_cutting">Upah Kerja Jahit (per baju) (Rp.):</label>
                    <input type="number" name="biaya_cutting" id="biaya_cutting" required readonly>
                @elseif (Auth::user()->role == 'packing')
                    <label for="biaya_packing">Upah Kerja Packing (per baju) (Rp.):</label>
    ...
    <script>
        $('select[name="kode_rencanaproduksi"]').on('change', function() {
            ...
            $.ajax({
                url: '/getRencanaProduksiDataCutting/' + kodeRencanaProduksi,
                type: 'GET',
                dataType: 'json',
        ...
        function updateFormWithRencanaProduksiData(data) {
            ...
            //Panggil fungsi  updateTotalBerhasilDangagal(); agar data-datanya ikut ke refresh
        ...
        function updateTotalBerhasilDangagal() {
            ...
            var nilaiSementara = jumlahRencanaProduksi - totalProduksiPakaian;

            //Masukan data ke sisaBelumProduksi
            ...
            if (totalBerhasilDangagal > nilaiSementara) {
                //alert('Total Berhasil + Gagal tidak boleh melebihi Jumlah Rencana Produksi - Total Produksi Pakaian.');
                alert('Tidak Boleh Melebihi Jumlah Pakaian Yang Belum Di Cutting');
